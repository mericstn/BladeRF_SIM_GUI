...
static int term_init()
{
    ...
    if (hStdin == INVALID_HANDLE_VALUE) {
        fprintf(stderr, "Error getting handle\n");
        return -1;
    }

    // Orijinal modu saklayýn
    ...
    new_mode &= ~ENABLE_LINE_INPUT;  // Canonical mode (satýr giriþi) kapalý
    new_mode &= ~ENABLE_ECHO_INPUT; // ECHO kapalý
...
void * tx_task_run(void *repeater_)
{
    ...
    if (!exit_early) {
        /* Call stream */
        status = bladerf_stream(repeater->tx_stream, BLADERF_CHANNEL_TX(0));
...
void * rx_task_run(void *repeater_)
{
    ...
    status = bladerf_stream(repeater->rx_stream, BLADERF_CHANNEL_RX(1));
    if (status < 0) {
...
static int init_device(struct repeater *repeater, struct repeater_config *config)
{
    ...
    status = bladerf_set_bandwidth(repeater->device, BLADERF_CHANNEL_TX(0),
    ...
    status = bladerf_set_bandwidth(repeater->device, BLADERF_CHANNEL_RX(1),
                                    config->bandwidth, &actual_value);
    ...
    status = bladerf_set_sample_rate(repeater->device, BLADERF_CHANNEL_TX(0),
    ...
    status = bladerf_set_sample_rate(repeater->device, BLADERF_CHANNEL_RX(1),
                                     config->sample_rate, &actual_value);
    ...
    status = bladerf_set_frequency(repeater->device,
                                   BLADERF_CHANNEL_TX(0), config->tx_freq);
    if (status < 0) {
    ...
    status = bladerf_set_frequency(repeater->device,
                                   BLADERF_CHANNEL_RX(1), config->rx_freq);
    if (status < 0) {
    ...
    status = bladerf_enable_module(repeater->device, BLADERF_CHANNEL_RX(1), true);
    if (status < 0) {
    ...
    status = bladerf_enable_module(repeater->device, BLADERF_CHANNEL_TX(0), true);
    if (status < 0) {
        bladerf_enable_module(repeater->device, BLADERF_CHANNEL_RX(1), false);
    ...
    status = bladerf_get_gain(repeater->device,
                    BLADERF_CHANNEL_RX(1), &repeater->gain_rx);
    if (status < 0) {
    ...
    bladerf_set_bias_tee(repeater->device, BLADERF_CHANNEL_RX(1), false);
   /* bladerf_set_stream_timeout(repeater->device, BLADERF_CHANNEL_RX(1), 250);
    bladerf_set_stream_timeout(repeater->device, BLADERF_CHANNEL_TX(0), 250);*/
...
static void deinit(struct repeater *repeater)
{
    ...
    if (repeater->device) {
        ...
        bladerf_enable_module(repeater->device, BLADERF_CHANNEL_RX(1), false);
        bladerf_enable_module(repeater->device, BLADERF_CHANNEL_TX(0), false);
